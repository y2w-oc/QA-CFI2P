...
class Embeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        if config.use_resnet_embedding:
            # <------ use resnet before partition ------>
            ...
            # <------ resnet and partition are parallel ------>
            self.mini_resnet = MiniResNet(inchannel=3, outchannel=config.embed_dim)
            self.patch_embeddings = nn.Conv2d(in_channels=3,
                                              out_channels=config.embed_dim,
                                              kernel_size=config.patch_size * 4,
                                              stride=config.patch_size)
    ...
    def forward(self, x):
        ...
        Output:
            embeddings: patch embedding add positional embedding (pixel proxy)
        """
        if self.config.use_resnet_embedding:
            resnet_out = self.embedding_layers[0](x)
            x = self.embedding_layers[1](resnet_out)
        else:
            resnet_out = self.mini_resnet(x)
...
class ImageTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        Input:
            x: source images
        Output:
            proxy_feat: pixel proxy with global context
            img_feat: image features
        """
        proxy_feat, img_feat = self.embeddings(x)

        for layer_block in self.sa_encoder_layers:
            proxy_feat = layer_block(proxy_feat)

        return proxy_feat, img_feat
